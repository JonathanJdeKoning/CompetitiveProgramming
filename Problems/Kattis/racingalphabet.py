...
from math import ceil, floor, sqrt, gcd, lcm, factorial, pi
...
def solve():
    ...
    ans += len(s)
    for c in s[1:]:
        dist = min(max(m[c], curr) - min(m[c],curr), 28-max(m[c], curr) + min(m[c],curr))
        ans += dist*((pi*4.2857142857142857142857)/radius)
        curr = m[c]
    return ans
