...
def solve():
    days, mn, mx = intspls()
    entries = [intpls() for x in range(days-1)]
    if mn not in entries and mx not in entries:
        print(-1)
        return

    if len([x for x in entries if x > mx or x < mn]) != 0:
        print(-1)
        return
    if mn not in entries:
        print(mn)
        return
    if mx not in entries:
        print(mx)
        return
